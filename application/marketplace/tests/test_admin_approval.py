...
class AdminApprovalTests(TestCase):
    ...
    def test_listing_detail_shows_condition_label(self):
        listing = Listing.objects.create(
            title="Condition Label Listing",
            description="Should display the human-readable label",
            price="12.00",
            listing_type=ListingType.PRODUCT,
            intent=ListingIntent.FOR_SALE,
            condition="like_new",
        ...
        response = self.client.get(
            reverse("listing_detail", kwargs={"listing_id": listing.listing_id}),
        )

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Like New")
        self.assertNotContains(response, "like_new")

    def test_edit_listing_preserves_selected_condition(self):
        listing = Listing.objects.create(
        ...
        self.client.force_login(self.seller)
        response = self.client.get(
            reverse("edit_listing", kwargs={"listing_id": listing.listing_id}),
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["selected_condition"], "fair")
        self.assertContains(response, '<option value="fair" selected>Fair</option>', html=True)

    def test_edit_listing_preserves_selected_category(self):
        other_category = Category.objects.create(
            category_name="Electronics",
            category_description="Devices",
        )
        listing = Listing.objects.create(
            title="Category Listing",
            description="Category should stay selected",
            price="40.00",
            listing_type=ListingType.PRODUCT,
            intent=ListingIntent.FOR_SALE,
            condition="good",
        ...
        self.assertContains(
            response,
            f'<option value="{other_category.category_id}" selected>Electronics</option>',
            html=True,
        )

    def test_create_listing_reposts_selected_condition_on_validation_error(self):
        self.client.force_login(self.seller)
        response = self.client.post(
            reverse("create_listing"),
            {
                "title": "",
                "description": "Missing title to force a validation error",
                "price": "9.99",
                "listing_type": ListingType.PRODUCT,
                "status": ListingIntent.FOR_SALE,
                "condition": "fair",
