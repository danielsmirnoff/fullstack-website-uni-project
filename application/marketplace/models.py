...
class User(AbstractUser):
    ...
    def save(self, *args, **kwargs):
        if not (self.sfsu_email or '').strip():
            if (self.email or '').strip():
                self.sfsu_email = self.email.strip().lower()
            elif (self.username or '').strip():
                self.sfsu_email = f"{self.username.strip().lower()}@sfsu.edu"

        if not (self.email or '').strip() and self.sfsu_email:
            self.email = self.sfsu_email

        if not (self.password_hash or '').strip() and self.password:
