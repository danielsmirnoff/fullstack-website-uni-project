...
def _normalize_condition_value(condition):
    if not condition:
        return ""
    return str(condition).strip().lower()


def _condition_display(condition):
    normalized_condition = _normalize_condition_value(condition)
    return dict(CONDITION_CHOICES).get(normalized_condition, condition or "Not specified")
...
def create_listing_view(request):
    ...
    def _create_form_context(post=None):
        post_condition = ""
        if post:
            post_condition = _normalize_condition_value(post.get("condition"))
        ctx = base_context()
        ctx.update({
            "user": user,
            ...
            "selected_condition": post_condition,
            "form": post or {},
        })
        return ctx
...
def edit_listing_view(request, listing_id):
    ...
    context.update({
        ...
        "listing_intents": ListingIntent.choices,
        "conditions": CONDITION_CHOICES,
        "selected_condition": _normalize_condition_value(listing.condition),
    })
...
def listing_detail_view(request, listing_id):
    ...
    context.update({
        ...
        "condition_display": _condition_display(listing.condition),
    })
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        # Update model fields
        # Fixes the regression where older users
        # without profiles could not save accounts
        if getattr(request.user, 'profile') is None:
